...
"""
def build_shift_dict(letter, shift):
    '''
    Creates a dictionary that can be used to apply a cipher to a letter.
    The dictionary maps every uppercase and lowercase letter to a
    character shifted down the alphabet by the input shift. The dictionary
    ...
    for i in range(len(string.ascii_lowercase)):
        dict_lower[string.ascii_lowercase[i]] = string.ascii_lowercase[i:] + string.ascii_lowercase[:i]

    dict_upper = {}
    for i in range(len(string.ascii_uppercase)):
        dict_upper[string.ascii_uppercase[i]] = string.ascii_uppercase[i:] + string.ascii_uppercase[:i]

    if letter not in (string.ascii_lowercase or string.ascii_uppercase) :
        return letter
    if letter in string.ascii_lowercase:
        return dict_lower[letter][shift]
    else:
        return dict_upper[letter][shift]
...
def apply_shift(word, shift):
    ...
    str = ''
    for letter in word:
        str += build_shift_dict(letter, shift)
    return str

print apply_shift('hello',25)
"""